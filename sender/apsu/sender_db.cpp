...
#include "apsu/sender_db_generated.h"
    ...
    namespace sender {
        ...
        void SenderDB::clear()
        {
            if (hashed_items_.size()) {
                APSU_LOG_INFO("Removing " << hashed_items_.size() << " items pairs from SenderDB");
            }
        ...
        OPRFKey SenderDB::strip()
        {
            ...
            APSU_LOG_INFO("SenderDB has been stripped");
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot return the OPRF key from a stripped SenderDB");
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot insert data to a stripped SenderDB");
            ...
            if (new_item_count) {
                APSU_LOG_INFO("Found " << new_item_count << " new items to insert in SenderDB");
            ...
            APSU_LOG_INFO("Finished inserting " << data.size() << " items in SenderDB");
        }
        ...
        void SenderDB::insert_or_assign(const vector<Item> &data)
        {
            ...
            if (is_labeled()) {
                APSU_LOG_ERROR("Attempted to insert unlabeled data but this is a labeled SenderDB");
                throw logic_error("failed to insert data");
            }
...
// TO DO : 
            ...
            APSU_LOG_INFO("Found " << hashed_data.size() << " new items to insert in SenderDB");
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot remove data from a stripped SenderDB");
...
// TO DO : 
            ...
            APSU_LOG_INFO("Finished removing " << data.size() << " items from SenderDB");
        }

        bool SenderDB::has_item(const Item &item) const
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot retrieve the presence of an item from a stripped SenderDB");
...
// TO DO : 
        ...
        {
            if (stripped_) {
                APSU_LOG_ERROR("Cannot retrieve a label from a stripped SenderDB");
                throw logic_error("failed to retrieve label");
            }
            if (!is_labeled()) {
                APSU_LOG_ERROR("Attempted to retrieve a label but this is an unlabeled SenderDB");
                throw logic_error("failed to retrieve label");
            }
            ...
            if (hashed_items_.find(hashed_item) == hashed_items_.end()) {
                APSU_LOG_ERROR("Cannot retrieve label for an item that is not in the SenderDB");
        ...
        size_t SenderDB::save(ostream &out) const
        {
            ...
            APSU_LOG_DEBUG("Start saving SenderDB");
            ...
            APSU_LOG_DEBUG("Finished saving SenderDB");
        ...
        pair<SenderDB, size_t> SenderDB::Load(istream &in)
        {
            ...
            APSU_LOG_DEBUG("Start loading SenderDB");
            ...
            if (!safe) {
                APSU_LOG_ERROR("Failed to load SenderDB: the buffer is invalid");
            ...
            try {
                ...
                APSU_LOG_ERROR("APSU threw an exception creating SenderDB: " << ex.what());
                throw runtime_error("failed to load SenderDB");
            }
            ...
            APSU_LOG_DEBUG("Finished loading SenderDB");
