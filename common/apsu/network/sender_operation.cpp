...
namespace apsu {
    ...
    namespace network {
        ...
        size_t plainResponse::save(std::ostream &out) const
        {
            ...
            sop_builder.add_request_type(fbs::Request_plainResponse);
            sop_builder.add_request(req.Union());
            auto sop = sop_builder.Finish();
            fbs_builder.FinishSizePrefixed(sop);
            out.write(
            ...
            std::istream &in, std::shared_ptr<seal::SEALContext> context)
        {
            ...
            if (!safe) {
             ...
             if (sop->request_type() != fbs::Request_plainResponse) {
                 throw runtime_error("unexpected operation type");
             }
             ...
             psi_result.resize(psi_data->size());
             
             copy(
                 psi_data->begin(), psi_data->end(), psi_result.begin());
